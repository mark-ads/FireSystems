...
class VideoItem(QThread):
    ...
    Третий вариант ПО для отображения видео.
    Является самым быстрым и производительным.
    Но пришлось от него отказаться, так как оказался несовместим с Qt.
    Нужно было сделать обработку видео исключительно на видеокарте.
    Проблема оказалась в том, что декодеры gstreamer для h264/h265
    работают на d3d11/d3d12, а QML может выводить только OpenGL.

    Собирал плагин для пайплайна Qml6d3d11sink, но он отказался неработоспособным на моей видеокарте.
    '''
