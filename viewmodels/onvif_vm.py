...
class OnvifVM(QObject):
    ...
    Содержит в себе ONVIF контролс.
    '''
    ...
    forwardCommandFloat = pyqtSignal(Command)

    def __init__(self, controls: OnvifControls):
        super().__init__()
        ...
        self.forwardCommandFloat.connect(self.controls.add_command)
        self.controls.onvifChanged.connect(self.update_current_params)
    ...
    def forward_float_command(self, slot: Literal['front', 'back'], command: str, value: float):
        ...
        self.forwardCommandFloat.emit(cmd)

    @pyqtSlot(str)
    def update_current_params(self, slot: Literal['front', 'back']):
        if slot == 'front':
            b, c, s = self.controls.front.get_current_params()
            self.frontBrightness = b
            self.frontContrast = c
            self.frontSaturation = s
            print('front:', b, c, s)
        else:
            b, c, s = self.controls.back.get_current_params()
