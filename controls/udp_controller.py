...
class UdpController(QObject):
    ...
    В начале приложения создается 2 объекта данного класса, на каждую сторону.
    После создания, объекты начинают ждать комманду.
    Объекты живут в основном потоке, так как нет блокирующих операций и все методы быстрые.
    Когда GUI готов - UdpVM помещает команду на подключение в очередь.
    При подключении применяет начальные настройки из конфиг.
    Отправляет команды через один общий UdpSender.
    '''
